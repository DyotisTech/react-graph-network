...
//     ).force("charge", (0, _d3Force.forceManyBody)().distanceMax(300).strength(-50)) // This adds repulsion between nodes. Play with the -400 for the repulsion strength
//     .on("tick", function () {
...
var Graph = function Graph(_ref) {
  ...
  (0, _react.useEffect)(function () {
    ...
    var link = svg.selectAll("._graphLine").data(data.links);
    var node = svg.selectAll("._graphNode").data(data.nodes);
    var simulation = (0, _d3Force.forceSimulation)(data.nodes).force(
      "center",
      (0, _d3Force.forceCenter)(
        svg._groups[0][0].parentElement.clientWidth / 2,
        svg._groups[0][0].parentElement.clientHeight / 2,
      ),
    ).force("link", (0, _d3Force.forceLink)() // This force provides links between nodes
    .id(function (d) {
      return d.id;
    }) // This provide the id of a node
    .distance(nodeDistance)
